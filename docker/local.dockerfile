...
RUN apt-get upgrade && apt-get update && \
    ...
    apt-get update && \
    DEBIAN_FRONTEND=noninteractive apt-get install -y curl python3.12 python3.12-venv python3.12-dev python3-pip ffmpeg build-essential \
    && apt-get clean \
...
RUN if [ "$NEED_MIRROR" = "1" ]; then \
    ...
    uv pip install --system dify_plugin || echo "dify_plugin installation failed, continuing..." && \
    python3.12 -c "from uv._find_uv import find_uv_bin;print(find_uv_bin());" || echo "uv test failed, continuing..." && \
    python3.12 -c "import tiktoken; encodings = ['o200k_base', 'cl100k_base', 'p50k_base', 'r50k_base', 'p50k_edit', 'gpt2']; [tiktoken.get_encoding(encoding).special_tokens_set for encoding in encodings]" || echo "tiktoken test failed, continuing..."
